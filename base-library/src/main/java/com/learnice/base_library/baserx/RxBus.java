package com.learnice.base_library.baserx;
...
public class RxBus {
    ...
    public <T> Observable<T> register(@NonNull Object tag) {
        ...
        LogUtil.debug("RxBus","register"+tag + "  size:" + subjectList.size());
        return subject;
    }
    ...
    public RxBus unregister(@NonNull Object tag,
        ...
        if (null != subjects) {
            ...
            if (isEmpty(subjects)) {
                subjectMapper.remove(tag);
                LogUtil.debug("RxBus","unregister"+ tag + "  size:" + subjects.size());
            }
        }
        return getInstance();
    }
    ...
    public void post(@NonNull Object tag, @NonNull Object content) {
        LogUtil.debug("RxBus","post"+ "eventName: " + tag);
        List<Subject> subjectList = subjectMapper.get(tag);
        if (!isEmpty(subjectList)) {
            for (Subject subject : subjectList) {
                subject.onNext(content);
                LogUtil.debug("RxBus","onEvent"+ "eventName: " + tag);
            }
        }
    }
